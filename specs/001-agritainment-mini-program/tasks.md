...
- [x] T054 实现定时任务 (每日库存重置、预约超时标记爽约、会员过期检查。卡券过期自动处理未实现)
...
| 任务 | 优先级 | 说明 |
|------|--------|------|
| T052 微信支付 | ⏭️ 本期不做 | 需要商户号和微信支付 API 接入，第二期实现 |
| T053 通知服务 | ✅ 已实现 | 微信订阅消息（预约/取消/服务下单通知） |
| T054 定时任务 | ✅ 已实现 | Spring @Scheduled 实现库存重置/预约超时/会员过期。卡券过期自动处理未实现 |
| T056 Nginx 部署 | LOW | 生产环境部署配置 |
| M2 摄像头冲突规则 | ⏭️ 本期不做 | 需要硬件接口配合，第二期实现 |
| M3 自动授权 | ⏭️ 本期不做 | 需要硬件接口配合，第二期实现 |
